        ...
        for idx in range(count):
            ...
            if flag == cli_name or (is_short and flag == short_name):
                ...
                if type_name == BOOL_NAME:
                    ...
                    var val = args[i]
                    ptr.unsafe_deinit_pointee()
                    ptr.unsafe_bitcast[String]().unsafe_write(val)
                    ...
                    var val = args[i]
                    ptr.unsafe_deinit_pointee()
                    ptr.unsafe_bitcast[Optional[String]]().unsafe_write(val)
                    ...
                    ptr.unsafe_deinit_pointee()
                    ptr.unsafe_bitcast[List[String]]().unsafe_write(parts^)
            ...
            for idx in range(count):
                ...
                if type_name == STRING_NAME:
                    if pos_count == positional_idx:
                        ...
                        var ptr = Pointer(to=field)
                        ptr.unsafe_deinit_pointee()
                        ptr.unsafe_bitcast[String]().unsafe_write(arg)
                    pos_count += 1
            ...
            if flag == cli_name or (is_short and flag == short_name):
                ...
                if type_name == BOOL_NAME:
                    ...
                    if i >= len(args):
                        raise Error("Missing value for --" + cli_name)
                    ptr.unsafe_deinit_pointee()
                    ptr.unsafe_bitcast[Int]().unsafe_write(atol(args[i]))
                elif type_name == STRING_NAME:
                    i += 1
                    if i >= len(args):
                        raise Error("Missing value for --" + cli_name)
                    ptr.unsafe_deinit_pointee()
                    ptr.unsafe_bitcast[String]().unsafe_write(args[i])
                elif type_name == FLOAT64_NAME:
                    i += 1
                    if i >= len(args):
                        raise Error("Missing value for --" + cli_name)
                    ptr.unsafe_deinit_pointee()
                    ptr.unsafe_bitcast[Float64]().unsafe_write(atof(args[i]))
                elif type_name == LIST_STRING_NAME:
                    i += 1
                    if i >= len(args):
                        raise Error("Missing value for --" + cli_name)
